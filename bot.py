...
async def start(client, message):
    await message.reply("Naber La Ben mal qw'nin boktan muzik botuyum qwe. ",
...
async def deezer(_, message):
    if len(message.command) < 2:
        await message.reply_text("Hangi şarkıyı çalacağım ?¿?¿")
        return
    text = message.text.split(None, 1)[1]
    query = text.replace(" ", "%20")
    hike = await message.reply_text("Aranıyor...")
    try:
        ...
        return
    await hike.edit("İndiriliyor...")
    song = await download_song(url)
    await hike.edit("Karşıya yükleniyor...")
...
async def play_track(client, message):
    ...
    if VOICE_CHATS and message.chat.id in VOICE_CHATS:
        text = f'▶️ Oynatılıyor... **{audio.title}**'
    else:
        try:
            group_call = GroupCall(client, input_filename)
            await group_call.start(message.chat.id)
        except RuntimeError:
            await message.reply('Grup çağrısını bulamadım :(')
            return
        VOICE_CHATS[message.chat.id] = group_call
    await a.edit(f'▶️ Oynatılıyor... **{audio.title}**')


@app.on_message(filters.command('gapat') & self_or_contact_filter)
async def stop_playing(_, message):
    group_call = VOICE_CHATS[message.chat.id]
    group_call.stop_playout()
    os.remove('downloads/vcbot/input.raw')
    await message.reply('Müziği gapattım.')


@app.on_message(filters.command('sesligel') & self_or_contact_filter)
async def join_voice_chat(client, message):
    ...
    if message.chat.id in VOICE_CHATS:
        await message.reply('Zaten seslideyim gerizekalı')
        return
    ...
    await message.reply('Sesliye giriş yapıldı ✅')


@app.on_message(filters.command('siktirgit') & self_or_contact_filter)
async def leave_voice_chat(client, message):
    chat_id = message.chat.id
    group_call = VOICE_CHATS[chat_id]
    await group_call.stop()
    VOICE_CHATS.pop(chat_id, None)
    await message.reply('Ok, sesliden çıkış yapıldı.')

app.start()
print('>>> qw Music Bot Çalıştırıldı.')
idle()
app.stop()
print('\n>>> qw Music Bot DUrduruldu.')
